...
func TestUnauthorizedAction(t *testing.T) {
	...
	if err := mgr.StartWork("ten_alpha", "act_unauth", intruder); !errors.Is(err, workflowaction.ErrUnauthorizedAction) {
	...
	if err := mgr.AttachEvidence("ten_alpha", "act_unauth", intruder, ev); !errors.Is(err, workflowaction.ErrUnauthorizedAction) {
		t.Errorf("expected ErrUnauthorizedAction on AttachEvidence by intruder, got: %v", err)
	}

	// 3. Non-owner attempts to submit for review
	if err := mgr.SubmitForReview("ten_alpha", "act_unauth", intruder, "done"); !errors.Is(err, workflowaction.ErrUnauthorizedAction) {
	...
	_ = mgr.AttachEvidence("ten_alpha", "act_unauth", "user_owner_alice", ev)
	ev2 := workflowaction.EvidenceAttachment{
		EvidenceID: "evd_002",
		TenantID:   "ten_alpha",
		Digest:     validEvidenceDigest2,
	}
	_ = mgr.AttachEvidence("ten_alpha", "act_unauth", "user_owner_alice", ev2)
	_ = mgr.SubmitForReview("ten_alpha", "act_unauth", "user_owner_alice", "ready for review")

	// 4. Non-reviewer attempts to reject
	if err := mgr.RejectReview("ten_alpha", "act_unauth", intruder, "rejected"); !errors.Is(err, workflowaction.ErrUnauthorizedAction) {
		t.Errorf("expected ErrUnauthorizedAction on RejectReview by intruder, got: %v", err)
	}

	// 5. Non-reviewer attempts to close
	if err := mgr.CloseAction("ten_alpha", "act_unauth", intruder, "closed"); !errors.Is(err, workflowaction.ErrUnauthorizedAction) {
		t.Errorf("expected ErrUnauthorizedAction on CloseAction by intruder, got: %v", err)
	}

	// Valid reviewer closes action
	if err := mgr.CloseAction("ten_alpha", "act_unauth", "user_reviewer_bob", "all good"); err != nil {
		t.Fatalf("close failed: %v", err)
	}

	// 6. Non-reviewer attempts to reopen
	if err := mgr.ReopenAction("ten_alpha", "act_unauth", intruder, "reopen"); !errors.Is(err, workflowaction.ErrUnauthorizedAction) {
...
func TestInsufficientEvidence(t *testing.T) {
	...
	ev1 := workflowaction.EvidenceAttachment{
		EvidenceID: "evd_first",
		TenantID:   "ten_alpha",
		Digest:     validEvidenceDigest1,
	}
	_ = mgr.AttachEvidence("ten_alpha", "act_insufficient", "user_owner_alice", ev1)
	...
	_ = mgr.AttachEvidence("ten_alpha", "act_insufficient", "user_owner_alice", ev2)

	// Now submission succeeds
	if err := mgr.SubmitForReview("ten_alpha", "act_insufficient", "user_owner_alice", "ready now"); err != nil {
		t.Fatalf("submit with 2 evidence failed: %v", err)
	}

	snap, _ := mgr.GetAction("ten_alpha", "act_insufficient")
...
func TestReviewRejectionAndResubmission(t *testing.T) {
	...
	_ = mgr.SubmitForReview("ten_alpha", "act_rejection", "user_owner_alice", "submitted")

	// Blank rejection reason rejected
	if err := mgr.RejectReview("ten_alpha", "act_rejection", "user_reviewer_bob", "   "); !errors.Is(err, workflowaction.ErrBlankReason) {
		t.Errorf("expected ErrBlankReason on empty rejection notes, got: %v", err)
	}

	if err := mgr.RejectReview("ten_alpha", "act_rejection", "user_reviewer_bob", "Photo 2 is blurry; retake inspection photo"); err != nil {
		t.Fatalf("unexpected error on valid rejection: %v", err)
	}
	snap, _ := mgr.GetAction("ten_alpha", "act_rejection")
	...
	if err := mgr.StartWork("ten_alpha", "act_rejection", "user_owner_alice"); err != nil {
	...
	// Resubmit
	if err := mgr.SubmitForReview("ten_alpha", "act_rejection", "user_owner_alice", "retake attached"); err != nil {
		t.Fatalf("resubmission failed: %v", err)
	}

	if err := mgr.CloseAction("ten_alpha", "act_rejection", "user_reviewer_bob", "verified clear photo"); err != nil {
		t.Fatalf("closure failed: %v", err)
	}

	snapClosed, _ := mgr.GetAction("ten_alpha", "act_rejection")
...
func TestReopenFlow(t *testing.T) {
	...
	_ = mgr.SubmitForReview("ten_alpha", "act_reopen", "user_owner_alice", "done")
	_ = mgr.CloseAction("ten_alpha", "act_reopen", "user_reviewer_bob", "accepted")

	// Attempting to reopen with blank reason
	if err := mgr.ReopenAction("ten_alpha", "act_reopen", "user_reviewer_bob", ""); !errors.Is(err, workflowaction.ErrBlankReason) {
		t.Errorf("expected ErrBlankReason, got: %v", err)
	}

	if err := mgr.ReopenAction("ten_alpha", "act_reopen", "user_reviewer_bob", "New safety hazard identified post-repair"); err != nil {
		t.Fatalf("reopen failed: %v", err)
	}

	snapReopened, _ := mgr.GetAction("ten_alpha", "act_reopen")
	...
	if err := mgr.ReopenAction("ten_alpha", "act_reopen", "user_reviewer_bob", "again"); !errors.Is(err, workflowaction.ErrInvalidPrecedingState) {
...
func TestOverdueHandling(t *testing.T) {
	...
	overdue, err := mgr.CheckOverdue("ten_alpha", "act_overdue")
	if err != nil || overdue {
	...
	snap, _ := mgr.GetAction("ten_alpha", "act_overdue")
...
func TestCrossTenantDenial(t *testing.T) {
	...
	err := mgr.AttachEvidence("ten_alpha", "act_cross", "user_owner_alice", evCross)
	if !errors.Is(err, workflowaction.ErrCrossTenantDenied) {
		t.Fatalf("expected ErrCrossTenantDenied, got: %v", err)
	}

	// Cross-tenant lookup returns ErrActionNotFound without leakage
	if _, err := mgr.GetAction("ten_bravo", "act_cross"); !errors.Is(err, workflowaction.ErrActionNotFound) {
		t.Fatalf("expected ErrActionNotFound on cross-tenant query, got: %v", err)
	}

	// Cross-tenant start work fails closed
	if err := mgr.StartWork("ten_bravo", "act_cross", "user_owner_alice"); !errors.Is(err, workflowaction.ErrActionNotFound) {
		t.Fatalf("expected ErrActionNotFound on cross-tenant start work, got: %v", err)
	}
...
func TestDuplicateAndConcurrentClosureDenial(t *testing.T) {
	...
	_ = mgr.SubmitForReview("ten_alpha", "act_closure", "user_owner_alice", "ready")
	...
	for i := 0; i < concurrency; i++ {
		...
		go func() {
			defer wg.Done()
			err := mgr.CloseAction("ten_alpha", "act_closure", "user_reviewer_bob", "closing")
			if err == nil {
	...
	if err := mgr.CloseAction("ten_alpha", "act_closure", "user_reviewer_bob", "re-closing"); !errors.Is(err, workflowaction.ErrActionClosed) {
...
func TestCompleteHistoryRequirement(t *testing.T) {
	...
	_ = mgr.SubmitForReview("ten_alpha", "act_history", "user_owner_alice", "review please")
	_ = mgr.RejectReview("ten_alpha", "act_history", "user_reviewer_bob", "need clearer photo")
	_ = mgr.StartWork("ten_alpha", "act_history", "user_owner_alice")
	_ = mgr.AttachEvidence("ten_alpha", "act_history", "user_owner_alice", workflowaction.EvidenceAttachment{
		EvidenceID: "evd_3",
		TenantID:   "ten_alpha",
	...
	_ = mgr.SubmitForReview("ten_alpha", "act_history", "user_owner_alice", "resubmitted")
	_ = mgr.CloseAction("ten_alpha", "act_history", "user_reviewer_bob", "verified and closed")
	_ = mgr.ReopenAction("ten_alpha", "act_history", "user_reviewer_bob", "reopened for inspection")

	snap, _ := mgr.GetAction("ten_alpha", "act_history")
