---
document_id: ARC-V040-EVD-001
title: v0.4.0 OSHE Inspect Evidence Capture, Metadata Minimization, Local Queue, Preview, Caption, Source Context, and Association Prework Baseline
...
credit_boundary: EVIDENCE_CAPTURE_PREWORK_SPECIFICATION_ONLY_NO_EXTERNAL_DELIVERY_OR_LIVE_OPERATIONAL_ENACTMENT
---

# v0.4.0 OSHE Inspect Evidence Capture, Metadata Minimization, Local Queue, Preview, Caption, Source Context, and Association Prework Baseline

## 1. Executive Summary & Governance Posture

### 1.1 Authority Baseline & Purpose
This architectural specification establishes the authoritative **Evidence Capture, Metadata Minimization, Local Queue, Preview, Caption, Source Context, and Record Association Prework Baseline** for **Milestone v0.4.0 - OSHE Inspect Private Alpha** within `OSHEThai/oshe-platform`. It fulfills the deliverable requirements of **GitHub Issue #128 (`[V040-I017] Implement Evidence Capture, Metadata, Local Queue, Preview, Caption, Source Context, and Record Association`)** under Roadmap Topic `V040-T04` and standing owner foundation decision `HDEC-V040-FOUNDATION-054`.
...
- Offline-capable local client queueing and transient secure preview rendering.
- Bounded supported media formats and size thresholds.
- Comprehensive fail-closed denial behavior and audit logging.

### 1.2 Unresolved Feature Dependency: `PENDING_DEPENDENCY_V040_I014`
In strict compliance with assignment directives, this specification provides the capture and association interface prework while explicitly preserving the backend compression, permanent object storage integration, and multipart transfer pipeline as **`PENDING_DEPENDENCY_V040_I014`** (governed under GitHub Issue #125 / `V040-I014`). Issue #128 remains open until full pipeline integration evidence is verified.
...
│  - state: Enum (INITIALIZED, QUEUED_LOCAL, UPLOADED, COMMITTED)        │
...
5. **Tenant Scope Invariant:** The parent entity's `tenant_id` must strictly match the caller's active tenant context (`ErrTenantMismatch`).
...
## 5. Client-Side Local Evidence Queue & Secure Preview

### 5.1 Local Queueing Model (`IndexedDB`)
Under intermittent or offline connectivity (`ARC-V040-MOBOFF-001`):
...
### 5.2 Ephemeral Preview Rendering
...
3. **Restricted Access:** Previews are accessible only to the active inspector on the device. Cross-session or unauthorized access is denied (`DENIAL_PREVIEW_UNAUTHORIZED`).

---

## 6. Fail-Closed Denial Reasons Matrix
...
## 7. Synthetic Multi-Scenario Fixture Matrix
...
## 8. Unresolved Policies & Dependencies Register

The following items are formally unselected or deferred pending future owner decisions:

1. **`PENDING_DEPENDENCY_V040_I014`:** The downstream implementation of permanent chunked storage, background compression, and upload pipeline integration is deferred to Issue #125 (`V040-I014`).
2. **`evidence_retention_policy: HUMAN_OWNED_UNSELECTED`:** Archival periods, legal spoliation rules, and automated purge workflows remain human-owned.
3. **`external_storage_provider_policy: HUMAN_OWNED_UNSELECTED`:** Cloud vendor selection and bucket configuration remain human-owned under Gate `H040-010`.

---

## 9. Governance Boundaries, Prohibitions & Non-Claims

In strict compliance with `HDEC-V040-FOUNDATION-054` and `ASN-V040-I017-EVIDENCE-CAPTURE-PREWORK-001`:
...
4. **Issue Closure Prohibition:** Issue #128 remains open following this delivery until all dependent pipeline components under `V040-I014` are integrated.
5. **Specification-Only Credit:** Delivery of this baseline confers documentation and architectural prework credit only; zero operational release, deployment, or residual-risk acceptance is claimed.
